	...
	private void writeFinalResult(TreeMap<ContentFile,String> tmap) throws IOException
	{
       ...
       bw=new BufferedWriter(new FileWriter(file_folder.concat("content123"+System.currentTimeMillis()+".csv")));
		...
		while(it.hasNext())
		{
			...
			/*sb.append(cf.getId());
			sb.append(delimiter);
			sb.append(cf.getPatientrefnum());
			sb.append(delimiter);
			sb.append(cf.getLipidprofile());
			sb.append(delimiter);
			sb.append(cf.getLcdlebel());
			sb.append(delimiter);
			sb.append(cf.getOpdlevel());
			sb.append(delimiter);
			sb.append(cf.getPcslevel());
			sb.append(delimiter);
			sb.append(cf.getSgtlevel());
			sb.append(delimiter);
			sb.append(cf.getChlratio());
			sb.append(cf.getAge());
